...
//      description = "Not recommended for JDBC 4 compliant drivers. Runs when a new database connection is established.",
    	      label = "连接健康测试查询",
    	      description = "不推荐JDBC驱动,数据库连接建立时候运行",
      displayPosition = 20,
      group = "LEGACY"
  )
...
//          "as Maximum Pool Size which effectively creates a fixed connection pool.",
    	      label = "最小空闲连接数",
    	      description = "最小空闲连接数，建议和最大连接池大小设置.",
      min = 0,
      defaultValue = "1",
      displayPosition = 20,
      group = "ADVANCED"
...
//      description = "Maximum time to wait for a connection to become available. Exceeding will cause a pipeline error.",
    	      label = "连接超时",
    	      description = "等待连接可用的最大时间.超过会导致任务错误",
      min = 1,
...
//          "remove idle connections.",
    	      label = "空闲超时",
    	      description = "连接池中空闲连接最大超时时间，0表示不移除空闲连接.",
      min = 0,
...
//          "means no maximum lifetime.",
    	      label = "最大连接存活时间",
    	      description = "连接池中最大连接存活时间.当达到时间时，它将从连接池中退出，0表示无最大存活时间 .",
      min = 0,
...
//      description = "Whether the connection should have property auto-commit set to true or not.",
    	      label = "自动提交",
    	      description = "设置自动提交，值为true 或者 not",
      defaultValue = "false",
      displayPosition = 55,
      group = "ADVANCED"
  )
...
//          "caution.",
    	      label = "只读连接",
    	      description = "只读连接，true为只读，false为非只读.",
      defaultValue = "true",
      displayPosition = 60,
      group = "ADVANCED"
  )
...
//        " added to connection pool.",
    	      label = "初始化查询",
    	      description = "在所有新的连接被创建时执行的SQL查询，在它们被添加到连接池之前.",
      displayPosition = 80,
      group = "ADVANCED"
  )
