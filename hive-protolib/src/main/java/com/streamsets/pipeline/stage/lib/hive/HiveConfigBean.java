...
public class HiveConfigBean {
  ...
  @ConfigDef(
      ...
      description = "连接Hive的JDBC地址，" +
          "格式: jdbc:hive2://<host>:<port>/<dbname>.",
...
//      description = "Additional properties to pass to the underlying JDBC driver.",
  ...
  @ConfigDef(
      ...
      label = "Hadoop配置文件目录",
      description = "配置文件hdfs-site.xml和hbase-site.xml所在的绝对路径或SDC资源目录下的相对目录",
...
//          " hdfs-site.xml and hive-site.xml files.",
  ...
  @ConfigDef(
      ...
      label = "Hadoop配置属性",
      description = "Hadoop配置属性. 此配置将覆盖配置文件中的属性",
...
//      description = "Additional configuration properties. Values here override values loaded from config files.",
  ...
  @ConfigDef(
      ...
      defaultValue = "-1",
      label = "最大缓存个数",
      description = "配置用于存储表相关信息的缓存大小。设置-1表示不限制。",
