...
public class HMSTargetConfigBean {
  ...
  @ConfigDef(
      ...
      label = "以Avro存储",
      description = "如果是Avro表，则包含于建表SQL子句中。勾选后，Avro元信息URL将不包括在查询中。",
...
//          " When selected, the Avro schema URL will not be included in the query.",
  ...
  @ConfigDef(
      ...
      label = "元信息目录路径",
      description = "如果指定，数据采集器将使用HDFS的位置用于序列化Avro元信息。" +
              "如果是相对路径，则相对于HDFS数据表数据所在路径",
...
//          " If the path does not start with '/' (relative) it will be relative to table data location in hdfs",
  ...
  @ConfigDef(
      ...
      type = ConfigDef.Type.STRING,
      label = "HDFS用户",
      description = "若设置，数据采集器将以此用户名写入HDFS中的avro元信息，" +
              "该用户必须是HDFS中的代理用户。",
//      label = "HDFS User",
