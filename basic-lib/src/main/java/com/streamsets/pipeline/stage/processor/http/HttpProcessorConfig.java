...
//      group = "DATA_FORMAT"
      ...
      label = "数据格式",
      description = "响应的数据格式。响应在被放入记录之前将被解析。",
      displayPosition = 1,
      group = "DATA_FORMAT"
  )
...
//      group = "HTTP"
          ...
          label = "HTTP请求方式表达式",
          description = "用于确定要使用的HTTP方法的表达式",
          displayPosition = 90,
          dependsOn = "httpMethod",
          elDefs = RecordEL.class,
          evaluation = ConfigDef.Evaluation.EXPLICIT,
...
//      group = "HTTP"
          ...
          label = "默认请求内容类型",
          defaultValue = HttpStageUtil.DEFAULT_CONTENT_TYPE,
          description = "Content-Type头与请求一起发送;如果该头部不存在，则使用该头部",
          displayPosition = 110,
          dependsOn = "httpMethod",
          elDefs = {RecordEL.class, VaultEL.class},
          evaluation = ConfigDef.Evaluation.EXPLICIT,
...
//      group = "HTTP"
          ...
          label = "频率限制(ms)",
          defaultValue = "0",
          description = "请求之间的时间（以毫秒为单位，0表示无限）。对速率有限的API非常有用。",
          displayPosition = 160,
          group = "HTTP"
  )
