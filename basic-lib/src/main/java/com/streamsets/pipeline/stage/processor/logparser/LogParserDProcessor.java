...
//      group = "LOG"
          ...
          label = "忽略控制字符",
          description = "只在必要时使用，因为它会影响读取速度",
          displayPosition = 15,
          group = "LOG"
  )
...
//    group = "LOG"
    ...
    label = "目标字段",
    description="设置解析的JSON数据的新字段的名称",
    displayPosition = 20,
    group = "LOG"
  )
...
//    mode = ConfigDef.Mode.PLAIN_TEXT
          ...
          label = "Grok模式定义",
          description = "定义你自己的grok模式，用来解析日志",
          displayPosition = 90,
          group = "LOG",
          dependsOn = "logMode",
          triggeredByValue = "GROK",
