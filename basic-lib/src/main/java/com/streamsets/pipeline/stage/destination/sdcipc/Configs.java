...
//      group = "RPC"
      ...
      defaultValue = "[]",
      label = "远程数据采集服务地址",
      description = "系统任务将在指定的主机和端口上启动。格式<host>:<port>.",
      displayPosition = 10,
      group = "RPC"
  )
...
//      group = "RPC"
      ...
      label = "远程数据采集服务标识",
      description = "用户定义的标识ID。必须与输出端配置的源任务管道标识匹配",
      displayPosition = 20,
      group = "RPC"
  )
...
//      min=0
      ...
      label = "重试间隔周期",
      description = "\n" +
              "如果设置为非零，每次重试将以该值为基数，按指数递增间隔周期。对于值10，" +
              "第一次重试将在10毫秒之后进行，第二次重试将在100(10^2)毫秒后进行，第三次重试将在1000(10^3)毫秒后进行，..." +
              "最大等待时间是5分钟。",
      displayPosition = 80,
      group = "ADVANCED",
      min=0
