...
//      description = "Port to listen on",
  ...
  @ConfigDef(
      ...
      label = "启用epoll",
      description = "每个端口使用多个接收线程,仅适用于64位Linux操作系统",
...
//      description = "User-specified TypesDB file. Overrides the included version.",
  ...
  @ConfigDef(
      ...
      defaultValue = "false",
      label = "转换时间戳/间隔",
      description = "将高分辨率时间格式的间隔和时间戳转换为UNIX时间（ms）.",
...
//      label = "Charset",
  ...
  @ConfigDef(
      ...
      label = "原始数据模式",
      description = "控制原始分包数据如何被处理的模式（基于字符或二进制）。此选择决定将创建什么类型的字段。",
//      label = "Raw Data Mode",
...
//      description = "The output field path to place the separated data values into.",
  ...
  @ConfigDef(
      ...
      type = ConfigDef.Type.MODEL,
      label = "多值处理",
      description = "如何处理在解析器应用分隔符后产生的多个值。",
...
//      description = "How to handle multiple values produced by the parser after applying the separator.",
  ...
  @ConfigDef(
      ...
      type = ConfigDef.Type.STRING,
      label = "数据分隔符",
      description = "用于分隔UDP数据包中的数据的字节。如果在应用此分隔符之后在包中找到多个值，" +
              "则按多值处理配置的方式处理。使用java语法指定字节Unicode文字（“\\uxxxx”）。" +
              "要捕获整个UDP数据包（即不使用任何分隔符拆分），请保留此空白。默认换行（000A）。",
