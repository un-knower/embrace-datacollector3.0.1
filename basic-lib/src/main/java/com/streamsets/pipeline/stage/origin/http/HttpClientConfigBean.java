...
//      group = "HTTP"
      defaultValue = "UTC",
      label = "请求体时区",
      description = "请求体使用的时区(如果使用EL表达式time()或者now())",
      displayPosition = 35,
      group = "HTTP"
  )
...
//      triggeredByValue = { "POST", "PUT", "DELETE" },
//      group = "HTTP"
          label = "默认请求ContentType",
          defaultValue = HttpStageUtil.DEFAULT_CONTENT_TYPE,
          description = "请求发送的Content-Type，在头信息中未设置时使用",
          displayPosition = 50,
          dependsOn = "httpMethod",
          elDefs = VaultEL.class,
          evaluation = ConfigDef.Evaluation.EXPLICIT,
