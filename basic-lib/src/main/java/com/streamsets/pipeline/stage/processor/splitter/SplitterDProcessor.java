...
//      group = "FIELD_SPLITTER"
      ...
      label = "分隔符",
      description = "正则表达式用于拆分字段。如果试图拆分RegEx元字符 \".$|()[{^?*+\\\", 则字符必须用\\",
      displayPosition = 20,
      group = "FIELD_SPLITTER"
  )
...
//      group = "FIELD_SPLITTER"
      ...
      label = "新字段",
      description="拆分后的字段。最后一个字段包含任何未分离的数据。",
      displayPosition = 30,
      group = "FIELD_SPLITTER"
  )
...
//      group = "FIELD_SPLITTER"
      ...
      label = "剩余拆分字段",
      description = "List集合字段，用于存储剩余拆分字段",
      displayPosition = 55,
      dependsOn = "tooManySplitsAction",
      triggeredByValue = "TO_LIST",
      group = "FIELD_SPLITTER"
