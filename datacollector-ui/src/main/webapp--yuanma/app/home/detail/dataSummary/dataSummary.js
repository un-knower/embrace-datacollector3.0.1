      ...
      if(pipelineMetrics && pipelineMetrics.histograms) {
        ...
        if(histogramData) {
          ...
          $scope.chartData = [{
            ...
            values: [
              ["平均值" , histogramData.mean ],
              ["标准偏差" , histogramData.stddev ],
    ...
    angular.extend($scope, {
      ...
      getRecordHeader: function(sampledRecord, index) {
        if(sampledRecord.matchedCondition) {
          return '<span class="condition-matched"> 匹配记录' + (index + 1) + ' </span>';
          // return '<span class="condition-matched"> Record' + (index + 1) + ' ( A Match ) </span>';
        } else {
          return '<span class="condition-not-matched"> 不匹配记录' + (index + 1) + ' </span>';
