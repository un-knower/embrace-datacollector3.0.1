...
public class PipelineLifecycleStageChooserValues implements ChooserValues {
  ...
  private static List<String> getOptions(List<StageDefinition> stageDefinitions, boolean value) {
    ...
    for (StageDefinition def : stageDefinitions) {
      if (value) {
        ...
        list.add(def.getLabel() + " (节点库: " + def.getLibraryLabel() + ")");
//        list.add(def.getLabel() + " (Library: " + def.getLibraryLabel() + ")");
  ...
  public static void setHandlingOptions(StageLibraryTask stageLibraryTask) {
    ...
    for (StageDefinition def : stageLibraryTask.getStages()) {
      if (def.isPipelineLifecycleStage()) {
        stageDefinitions.add(def);
      }
    }

    Collections.sort(stageDefinitions, (o1, o2) -> o1.getLabel().compareToIgnoreCase(o2.getLabel()));

    PipelineLifecycleStageChooserValues.values = getOptions(stageDefinitions, true);
    PipelineLifecycleStageChooserValues.labels = getOptions(stageDefinitions, false);
  }
