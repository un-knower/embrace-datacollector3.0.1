...
//    label = "Hadoop FS URI",
  ...
  @ConfigDef(
    ...
    type = ConfigDef.Type.STRING,
    label = "HDFS用户名",
    description = "若设置，数据采集器将以此用户名写入HDFS，该用户必须是HDFS中的代理用户。",
//    label = "HDFS User",
...
//    label = "Kerberos Authentication",
  ...
  @ConfigDef(
    ...
    defaultValue = "",
    label = "HDFS配置文件目录",
    description = "HDFS配置文件core-site.xml和hdfs-site.xml所在的SDC资源文件目录或软连接",
...
//    description = "An SDC resource directory or symbolic link with HDFS configuration files core-site.xml and hdfs-site.xml",
  ...
  @ConfigDef(
    ...
    label = "Hadoop文件系统配置项",
    description = "用于配置Hadoop文件系统属性. 优先级高于Hadoop文件系统配置文件中的属性",
...
//      description = "File name suffix e.g.'txt'",
  ...
  @ConfigDef(
    ...
    defaultValue = "false",
    label = "路径在头信息中",
    description = "头信息中以'" + HdfsTarget.TARGET_DIRECTORY_HEADER + "'属性记录目录信息",
...
//    description = "The directory is defined by the '" + HdfsTarget.TARGET_DIRECTORY_HEADER + "' record header attribute instead of the Directory Template configuration property.",
  ...
  @ConfigDef(
    ...
    label = "目录模板",
    description = "输出目录命名模板. 变量支持如下：${yyyy()}, ${mm()}, ${dd()}, " +
      "${hh()}, ${mm()}, ${ss()}，以及字段值，格式：{record:value(“/field”)}.",
...
//      "processing time, enter ${time:now()}. To use field values, use '${record:value(\"<filepath>\")}'.",
  ...
  @ConfigDef(
    ...
    defaultValue = "0",
    label = "文件最大记录数",
    description = "当文件中记录数超过此值时，则触发创建新文件.设置0表示不生效",
...
//    description = "Number of records that triggers the creation of a new file. Use 0 to opt out.",
  ...
  @ConfigDef(
    ...
    defaultValue = "0",
    label = "文件最大字节(MB)",
    description = "当文件大小超过此值时，则触发创建新文件. 设置0表示不生效",
...
//    description = "Exceeding this size triggers the creation of a new file. Use 0 to opt out.",
  ...
  @ConfigDef(
    ...
    label = "最大空闲时间",
    description = "文件保持空闲的最大时间，" +
            "超过此时间仍没有记录写入则关闭文件。" +
            "支持按表达式方式-分钟(MINUTES)、小时(HOURS)进行定义，" +
            "如：${1 * HOURS}，若仅输入数字时，则以秒为单位",
//    label = "Idle Timeout",
...
//    description = "",
  ...
  @ConfigDef(
    ...
    defaultValue = "${uuid()}",
    label = "序列文件记录主键",
    description = "Hadoop序列文件中记录的主键. 格式： " +
      "'${record:value(\"<字段路径>\")}'或'${uuid()}'",
...
//    description = "Compression type if using a CompressionCodec",
  ...
  @ConfigDef(
    ...
    defaultValue = "${1 * HOURS}",
    label = "迟到时间(秒)",
    description = "配置记录被写入到相应的HDFS目录的时间限制，" +
            "超出此时限的记录将被写入当前记录文件。" +
            "时限支持按表达式方式-分钟(MINUTES)、小时(HOURS)进行定义，" +
            "如：${1 * HOURS}，若仅输入数字时，则以秒为单位",
...
//      "'${30 * MINUTES}'",
  ...
  @ConfigDef(
    ...
    defaultValue = "false",
    label = "启用滚动属性",
    description = "在处理具有指定滚动属性的记录时关闭当前文件并创建一个新文件。",
...
//    description = "Action for records considered late.",
  ...
  @ConfigDef(
    ...
    label = "迟到记录目录模板",
    description = "创建迟到文件目录的模板. 支持如下变量：${YYYY()}, ${MM()}, " +
      "${DD()}, ${hh()}, ${mm()}, ${ss()}.",
...
//    description = "Data Format",
  ...
  @ConfigDef(
    ...
    defaultValue = "true",
    label = "HDFS权限验证",
    description = "若勾选, HDFS目标节点会采用在目标目录创建测试文件的方式检查是否具有操作权限",
...
//    description = "When checked, HDFS destination will create test file in configured target directory to verify access privileges.",
  ...
  @ConfigDef(
      ...
      evaluation = ConfigDef.Evaluation.EXPLICIT,
      label = "权限表达式",
      description = "确定目标文件权限的表达式。采用八进制/符号表示权限。",
