...
public class ProxyAuthenticator extends LoginAuthenticator {
  ...
  //add by bingo jwt
  private JwtLdapLogin jwtLdapLogin;
  ...
  public ProxyAuthenticator(LoginAuthenticator authenticator, RuntimeInfo runtimeInfo, Configuration conf) {
    ...
    this.conf = conf;
    //add by bingo jwt
    javax.security.auth.login.Configuration  config = java.security.AccessController.doPrivileged
            (new java.security.PrivilegedAction<javax.security.auth.login.Configuration>() {
                public javax.security.auth.login.Configuration run() {
                    return javax.security.auth.login.Configuration.getConfiguration();
                }
            });
   String isJwt = this.conf.get("jwt.is", "0");
   if("1".equals(isJwt)){
       AppConfigurationEntry []  entry = config.getAppConfigurationEntry("ldap");
       String pemPath = runtimeInfo.getConfigDir()+"/"+conf.get("jwm.pem", "cert.pem");
       String jwtCookieName = conf.get("jwt.cookie.name", "hadoop-jwt");
       jwtLdapLogin = new JwtLdapLogin(entry[0].getOptions(),pemPath,jwtCookieName);
   }
  }
  ...
  public Authentication validateRequest(ServletRequest req, ServletResponse res, boolean mandatory)
    ...
    if (authentication == null) {
      ...
      String isJwt = this.conf.get("jwt.is", "0");
      if("1".equals(isJwt)){
         String username = jwtLdapLogin.getUserName(request);
         String jwtRedirect = this.conf.get("jwt.redirect", "");
         if(!StringUtils.isEmpty(username)){
             String password = jwtLdapLogin.getUserCredential(username);
             ...
             if (userIdentity != null) {
                 ...
                 try {
                   response.sendRedirect(jwtRedirect);
               } catch (IOException e) {
      ...
      }else if (this.authenticator instanceof FormAuthenticator) {
