...
public enum CsvMode implements Label {
  CSV("默认CSV格式(忽略空行)", CSVFormat.DEFAULT),
  RFC4180("RFC4180 CSV格式", CSVFormat.RFC4180),
  EXCEL("MS Excel CSV格式", CSVFormat.EXCEL),
  MYSQL("MySQL CSV格式", CSVFormat.MYSQL),
  TDF("Tab分割格式", CSVFormat.TDF),
  POSTGRES_CSV("Postgres CSV格式", CSVFormat.POSTGRESQL_CSV),
  POSTGRES_TEXT("Postgres Text格式", CSVFormat.POSTGRESQL_TEXT),
  CUSTOM("自定义格式", null)
