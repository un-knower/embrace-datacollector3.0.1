...
public enum FileRollMode implements Label {
  REVERSE_COUNTER("以数字结尾的滚动文件", LogRollModeFactory.REVERSE_COUNTER),
  DATE_YYYY_MM("以.yyyy-MM结尾的文件", LogRollModeFactory.DATE_YYYY_MM),
  DATE_YYYY_MM_DD("以.yyyy-MM-dd结尾的文件", LogRollModeFactory.DATE_YYYY_MM_DD),
  DATE_YYYY_MM_DD_HH("以.yyyy-MM-dd-HH结尾的文件", LogRollModeFactory.DATE_YYYY_MM_DD_HH),
  DATE_YYYY_MM_DD_HH_MM("以.yyyy-MM-dd-HH-mm结尾的文件", LogRollModeFactory.DATE_YYYY_MM_DD_HH_MM),
  DATE_YYYY_WW("以.yyyy-ww结尾的文件", LogRollModeFactory.DATE_YYYY_WW),
  ALPHABETICAL("按字母顺序排序的文件", LogRollModeFactory.ALPHABETICAL),
  PATTERN("格式匹配的文件", new PeriodicFilesRollModeFactory()),
