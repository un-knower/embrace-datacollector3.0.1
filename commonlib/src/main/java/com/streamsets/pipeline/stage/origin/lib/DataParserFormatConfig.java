...
//          "Archive formats 7z, ar, arj, cpio, dump, tar and zip ar supported.",
        label = "压缩格式",
        description = "支持的压缩格式：gzip, bzip2, xz, lzma, Pack200, DEFLATE, Z；<br>" +
                "支持的归档格式：7z, ar, arj, cpio, dump, tar, zip",
      defaultValue = "NONE",
...
//      description = "One or more characters. Leading and trailing spaces are stripped.",
      label = "自定义分隔符",
      description = "单个或多个字符. 头部和结尾的空格会被去除。",
      displayPosition = 344,
      group = "DATA_FORMAT",
...
//      description = "When false, rows with more columns than the header are sent to error.",
      label = "是否允许附加列",
      description = "若未勾选,列多于首行的记录行将被发送至错误处理节点",
      displayPosition = 385,
      group = "DATA_FORMAT",
      dependencies = {
...
//      description = "When checked, configured string constant will be converted into NULL field.",
  ...
  @ConfigDef(
      ...
      defaultValue = "\\\\N",
      label = "空值常量",
      description = "遇到该字符串时,将被解析为空(NULL)",
      displayPosition = 437,
      group = "DATA_FORMAT",
      dependsOn = "parseNull",
      triggeredByValue = "true"
...
//      description = "Map groups in the regular expression to field paths",
      label = "字段路径-正则组映射",
      description = "将正则表达式中的映射组映射到字段路径",
      displayPosition = 510,
      group = "DATA_FORMAT",
      dependsOn = "logMode",
      triggeredByValue = "REGEX"
...
//      description = "",
  ...
  @ConfigDef(
      ...
      defaultValue = "50",
      label = "堆栈信息长度",
      description = "同一消息中任何与预期模式不匹配的行都将被视为堆栈跟踪信息，并被裁剪为指定的行数。",
...
//      description = "Where to load the Avro Schema from.",
  ...
  @ConfigDef(
      ...
      type = ConfigDef.Type.TEXT,
      label = "Avro元信息",
      description = "重写包含在数据中的元信息。可以使用loadresource函数加载文件中的元信息。",
//      label = "Avro Schema",
...
//      label = "Schema ID",
  ...
  @ConfigDef(
      ...
      defaultValue = "",
      label = "Protobuf描述文件",
      description = "Protobuf描述文件(.desc)相对路径(相对于数据采集服务资源目录)",
...
//      description = "Protobuf Descriptor File (.desc) path relative to SDC resources directory",
  ...
  @ConfigDef(
      ...
      type = ConfigDef.Type.STRING,
      defaultValue = "",
      description = "消息类型命名格式：<packageName>.<messageTypeName>",
      label = "消息类型",
...
//      label = "Message Type",
  ...
  @ConfigDef(
      ...
      defaultValue = "true",
      label = "是否拆分消息",
      description = "当消息体前部带有消息大小信息时，应该勾选。若未勾选，源文件或Kafka消息仅作为一条消息",
...
//    description = "User-specified TypesDB file. Overrides the included version.",
  ...
  @ConfigDef(
    ...
    defaultValue = "false",
    label = "是否转换时间格式",
    description = "将Hi-Res间隔时间和时间戳转换为UNIX时间(ms)",
...
//      description = "Size of the Buffer used to copy the file.",
  ...
  @ConfigDef(
      ...
      defaultValue = "-1",
      label = "每秒传输速率",
      description = "控制文件传输占用的带宽，使用<= 0选择退出。默认单位是B/s",
