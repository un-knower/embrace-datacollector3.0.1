...
public class S3TargetConfigBean {
  ...
  @ConfigDef(
      ...
      defaultValue = "",
      label = "分区前缀",
      description = "写入的分区。若该分区不存在则自动创建",
...
//      description = "Partition to write to. If the partition doesn't exist on Amazon S3, it will be created.",
  ...
  @ConfigDef(
      ...
      defaultValue = "UTC",
      label = "时区",
      description = "用于解析基于时间的分区前缀的日期时间的时区",
...
//      description = "Time zone to use to resolve the date time of a time-based partition prefix",
  ...
  @ConfigDef(
      ...
      label = "时间基准",
      description = "记录采用的时间基准，输入评估时间的表达式。" +
              "使用当前时间，输入${time:now()}；" +
              "使用字段值，输入${record:value(\"<filepath>\")}。",
//      label = "Time Basis",
...
//          "processing time, enter ${time:now()}. To use field values, use '${record:value(\"<filepath>\")}'.",
  ...
  @ConfigDef(
    ...
    type = ConfigDef.Type.STRING,
    defaultValue = "sdc",
    description = "上传至Amazon S3的对象的名称前缀",
    label = "对象名前缀",
...
//    label = "Object Name Prefix",
  ...
  @ConfigDef(
      required = false,
      type = ConfigDef.Type.STRING,
      description = "上传至Amazon S3的对象的名称后缀. 例如：txt",
      label = "对象名后缀",
